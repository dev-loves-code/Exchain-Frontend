...
import { Loader2, Edit, ArrowLeft, AlertCircle, CheckCircle } from 'lucide-react';
  ...
  if (profile.agent_profile) {
    return {
      ...
      agent_profile: {
        ...
        working_hours_start: normalize(profile.agent_profile.working_hours_start),
      ...
      <div className="space-y-6">
        ...
        {isAgent && profile.agent_profile && (
          <>
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-800">Agent Profile</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <InfoCard
                  ...
                  value={profile.agent_profile.commission_rate ? `${profile.agent_profile.commission_rate}%` : null}
                />
              </div>
            </div>

            {/* Map Section */}
            {profile.agent_profile.latitude && profile.agent_profile.longitude && (
              <div className="bg-white rounded-xl border border-gray-200 p-6">
...
const EditProfileView = ({ profile, onCancel, onUpdate }) => {
  ...
  const handleUserSubmit = async (userData) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.errors) {
          setAlert({ type: 'error', message: 'Please fix the validation errors' });
        } else {
  ...
  const handleAgentSubmit = async (agentData) => {
    ...
    try {
      ...
      if (result.success) {
        setAlert({ type: 'success', message: 'Agent profile updated successfully!' });
        setTimeout(() => {
          onUpdate(result.data);
        }, 1500);
      } else {
