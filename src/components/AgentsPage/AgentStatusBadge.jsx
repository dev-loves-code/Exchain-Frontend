...
export default function AgentStatusBadge({ status, showFullText = false }) {
  ...
  return (
    <div className={`px-2 py-1 rounded-full text-xs font-medium flex items-center ${getStatusColor(status)}`}>
      {getStatusIcon(status)}
      <span className={`ml-1 capitalize ${showFullText ? '' : 'hidden sm:inline'}`}>
        {status}
      </span>
    </div>
  );
